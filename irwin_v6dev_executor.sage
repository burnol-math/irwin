...
#       usually maxworkers of them at a time.  It computes
...
def _v6_setup_para_recurrence(touslescoeffs, Gammas, PuissancesDeD,
    ...
    def _v6_para_recurrence(m, step, useparallel):
        ...
        if ((M - 400) % 500 < maxworkers):
            ...
            # MEMO: step is usually maxworkers but may be strictly less
            results = list(executor.map(_v6_ukm_partial,
                                        [(a, M + a,
                                          PascalRows[a],
                                          Gammas,
                                          PuissancesDeD,
...
def _v6_map_beta_notimes(Mmax, IndexToR, maxblock, executor):
    ...
    if extra < maxworkers:
        def map__v6_beta(j):
            ...
            results = list(executor.map(_v6_beta, inputdata))
...
def _v6_map_beta_withtimes(Mmax, IndexToR, maxblock, executor):
    ...
    def map__v6_beta(j):
        ...
        for rep in range(Mmax // mSize):
            ...
            # We call the parallelized _v6_beta with exactly maxworkers
            # arguments.
...
def irwin(b, d, k,
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        for P in range(Q):
            useparallel = _v6_para_recurrence(m, maxworkers, useparallel)
            m += maxworkers
...
The variable "maxworkers" sets how many processes to use
with concurrent.futures.ProcessPoolExecutor(max_workers=maxworkers).
