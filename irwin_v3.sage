...
with two main enhancements relevant to targeting 1000 or
...
def irwin(b, d, k,
    ...
    for m in range(2, Mmax+1):
        ...
        PascalRow = [1]
        halfm = m//2
        PascalRow.extend([prevPascalRow[j-1]+prevPascalRow[j]
                          for j in range(1, halfm)])
        if not (m&1):
            L = PascalRow.copy()
        ...
        for j in range(1,k+1):
            cm.append((sum(PascalRow[i]*Rm(lesgammas[i])*
                           Rm(touslescoeffs[m-i][j]) for i in range(1,m+1))
                       + cm[-1]
                       + sum(PascalRow[i]*Rm(lespuissancesded[i])
                             *Rm(touslescoeffs[m-i][j-1]) for i in range(1,m+1))
                       )
                      / Rm(b**(m+1)-bmoinsun)
                      )
    ...
    for j in range(0 if all else k, k+1):
        ...
        bubu = touslescoeffs[Mmax][j] * sum(1/Rm(n ** (Mmax+1))
                                            for n in maxblock[0])

        if j >= 1:
            bubu += touslescoeffs[Mmax][j-1] * sum(1/Rm(n ** (Mmax+1))
                                                   for n in maxblock[1])
        if j >= 2:
            bubu += touslescoeffs[Mmax][j-2] * sum(1/Rm(n ** (Mmax+1))
                                                   for n in maxblock[2])
        if (level > 2) and (j >= 3):
            bubu += touslescoeffs[Mmax][j-3] * sum(1/Rm(n ** (Mmax+1))
                                                   for n in maxblock[3])
        if (level == 4) and (j >= 4):
            bubu += touslescoeffs[Mmax][j-4] * sum(1/Rm(n ** (Mmax+1))
                                                   for n in maxblock[4])
        ...
        for m in range(Mmax-1, 0, -1):  # last one is m=1
            Rm = IndexToR[m]
            bubu = Rm(-bubu)
            bubu += touslescoeffs[m][j] * sum(1/Rm(n ** (m+1))
                                              for n in maxblock[0])

            if j >= 1:
                bubu += touslescoeffs[m][j-1] * sum(1/Rm(n ** (m+1))
                                                    for n in maxblock[1])
            if j >= 2:
                bubu += touslescoeffs[m][j-2] * sum(1/Rm(n ** (m+1))
                                                    for n in maxblock[2])
            if (level > 2) and (j >= 3):
                bubu += touslescoeffs[m][j-3] * sum(1/Rm(n ** (m+1))
                                                    for n in maxblock[3])
            if (level == 4) and (j >= 4):
                bubu += touslescoeffs[m][j-4] * sum(1/Rm(n ** (m+1))
                                                    for n in maxblock[4])
...
def irwinpos(b, d, k,
    ...
    for m in range(2, Mmax+1):
        ...
        cm = [ (Rm(b ** (m+1))
                + sum(PascalRow[i]*Rm(lesgammasprime[i])*
                      Rm(touslescoeffs[m-i][0]) for i in range(1,m+1))
                )
                / Rm(b**(m+1) - bmoinsun)
              ]
    ...
    for j in range(0 if all else k, k+1):
        ...
        Rm = IndexToR[-1]
        bubu = touslescoeffs[Mmax][j] * sum(1/Rm(n ** (Mmax+1))
                                            for n in maxblockshifted[0])

        if j >= 1:
            bubu += touslescoeffs[Mmax][j-1] * sum(1/Rm(n ** (Mmax+1))
                                                   for n in maxblockshifted[1])
        if j >= 2:
            bubu += touslescoeffs[Mmax][j-2] * sum(1/Rm(n ** (Mmax+1))
                                                   for n in maxblockshifted[2])
        if (level > 2) and (j >= 3):
            bubu += touslescoeffs[Mmax][j-3] * sum(1/Rm(n ** (Mmax+1))
                                                   for n in maxblockshifted[3])
        if (level == 4) and (j >= 4):
            bubu += touslescoeffs[Mmax][j-4] * sum(1/Rm(n ** (Mmax+1))
                                                   for n in maxblockshifted[4])

        if verbose:
            lastterm = bubu  # the Feb 2024 version had a bug here
                             # doing -bubu if Mmax odd
...
General information is also available in the irwin_v3_docstring
variable.
"""
          )
