...
__date__     = "2025/05/07"
__filename__ = "irwin_v6.sage"

import multiprocessing as mp
# on my system default within SageMath will be 'spawn'
# and the computation times are higher in my context.
...
try:
    _ = numworkers
    numworkersinfostring = ("numworkers variable already existed "
                            f"and has value {numworkers}.")
except NameError:
    numworkers = 8
    numworkersinfostring = ("numworkers variable has been created "
...
- Parallelization, using multiprocessing library.
...
def _fillin_irwin_docstring():
    def dec(obj):
        obj.__doc__ = obj.__doc__.format(
            irwin_v6_fn_docstring.format('u_{j;m}', 'irwin') if
            (obj.__name__ == 'irwin') else
            irwin_v6_fn_docstring.format('v_{j;m}', 'irwinpos')
        )
        return obj
    return dec

irwin_v6_fn_docstring = """:param int b: the integer base
...
# @parallel(ncpus=numworkers)
# def _v6_u0m_partial(a, m, P, G, D, T, R):
#     # m -a will be the same value in all simultaneous calls
#     return sum(P[i]*R(G[i])*R(T[m - i][0]) for i in range(a, m + 1))


def _v6_umtimeinfo(single_ns, multi_ns, para, wrkrs, M, s):
    """Informational helper to be shared between irwin() and irwinpos()
    """
    multi = multi_ns * 1e-9
    ...
    if multi_ns < single_ns:
        if para:
            ...
            print(f"... basculement car {single:.3f}s>{multi:.3f}s"
                  f" en parallèle (numworkers={wrkrs}, {M}<m<={M+s})")
    else:
        if para:
...
# MEMO: this will be executed "in parallel" for multiple "a"'s,
#       usually numworkers of them at a time.  It computes
#       only partially the recurrent formulas, which are
#       corrected in a non-parallel manner later on.
# m - a will be the same value in all "parallel" calls
def _v6_ukm_partial(args):
    a, m, P, G, D, T, R, k = args
...
# MAIN AUXILIARY TO COMPUTE THE RECURRENT COEFFICIENTS.
# It actually defines (in a manner which can be used both by irwin()
# and irwinpos()) the interface to the actual workers but also
# does some extra corrections of its own to realize the magic
# of parallelizing a linear one-by-one ("full") recurrence.
# Hence the "pool" last argument.
def _v6_setup_para_recurrence(touslescoeffs, Gammas, PuissancesDeD,
                              PascalRows, IndexToR, b, bmoinsun, k,
                              showtimes, persistentpara, is_for_vm, pool):
    """Defines an auxiliary to parallelize the J-F. B. recurrence
    """
    def _v6_para_recurrence(m, step, useparallel):
        """Mostly called with step a multiple of numworkers
        """
        del PascalRows[:-1]
        for i in range(step):
            m += 1
        ...
        M = m - step  # original value of m
        # A bit complex logic here for a non-used thing which was
        # to test if parallel processing was still useful, but
        # the persistentpara Boolean was made True by default
        # and this was from time of using @parallel which induced
        # lots of overhead.  We keep the logic for a while.
        if ((M - 400) % 500 < numworkers):
            starttime_ns = time.perf_counter_ns()
            # MEMO: step is usually numworkers but may be strictly less
            results = pool.map(_v6_ukm_partial, [(a, M + a,
                                                  PascalRows[a],
                                                  Gammas,
                                                  PuissancesDeD,
                                                  touslescoeffs,
            ...
            if useparallel and persistentpara:
                ...
                # redo the computation in a non-parallel manner
                starttime_ns = time.perf_counter_ns()
                m = M
                ukm_partial = [ None ]
                ...
                if showtimes:
                    _v6_umtimeinfo(singletime_ns, multitime_ns,
                                   useparallel,
                                   numworkers, M, step)
                # decide now if to toggler in (or out) of parallel mode
        ...
        # update status
        return useparallel
    return _v6_para_recurrence


# MAIN AUXILIARIES TO PARALLELIZE COMPUTATION OF THE beta(m+1)'s
def _v6_beta_aux(m, R, nblock):
    return sum(1/R(n ** (m+1)) for n in nblock)

# This is the worker.
def _v6_beta(args):
    start, end, IR, nblock = args
    return list(_v6_beta_aux(m, IR[m], nblock)
                for m in range(start, end, numworkers))

# This defines the procedure doing the interface to using the workers and
# reassembling the results.
def _v6_map_beta_notimes(Mmax, IndexToR, maxblock, pool):
    extra = numworkers - ( Mmax % numworkers )
    if extra < numworkers:
        def map__v6_beta(j):
            L = [0]
            inputdata = [(i,
                          ...
                          IndexToR,
                          maxblock[j])
                         for i in range(1, 1 + numworkers)]
            results = pool.map(_v6_beta, inputdata)
            for j in range(1, extra + 1):
                results[-j].append(None)
            L.extend(x for xs in zip(*results) for x in xs)
            return L[:-extra]
    else:
        def map__v6_beta(j):
            L = [0]
...
def _v6_map_beta_withtimes(Mmax, IndexToR, maxblock, pool):
    """Inform user of how things are going every 1000 m's, more or less
    """
    # We want to display some visual sign of progress.
    # Find the largest multiple of numworkers at most 1000,
    # do something reasonable if numworkers is big
    q = max(1000 // numworkers, 32)
    mSize = q * numworkers
    def map__v6_beta(j):
        print(f"... ({j} occ.) ", end = "", flush = True)
        starttime = time.perf_counter()
        lasttime = starttime
        mbegin = 1  # will remain congruent to 1 modulo numworkers
        mend = 1    # this one also
        L = [0]
        for rep in range(Mmax // mSize):
            mend   = mbegin + mSize
            # In this loop, mSize is a multiple of q.
            # We call the parallelized _v6_beta with exactly numworkers
            # arguments.
        ...
        if mend < Mmax+1:
            extra = numworkers - ( (Mmax + 1 - mend) % numworkers )
            inputdata = [(mend + i,
                          Mmax + 1,
                          IndexToR,
                          maxblock[j])
...
def _v6_setup_realfields(nbdigits, PrecStep, b, level, Mmax=-1):
...
def _v6_setup_blocks(b, d, level):
...
def irwin(b, d, k,
    ...
    (nbbits,
     ...
     Mmax,
     IndexToR,
     NbOfPrec) = _v6_setup_realfields(nbdigits, PrecStep, b, level, Mmax)
    ...
    blocks = _v6_setup_blocks(b, d, level)
    block1 = blocks[0]
    block2 = blocks[1]
    if level >2:
        block3 = blocks[2]
    ...
    if k > 0:
        ...
        # we need the name to be defined when calling _v6_ukm_partial
    ...
    with mp.Pool(processes=numworkers) as pool:
        _v6_para_recurrence = _v6_setup_para_recurrence(touslescoeffs,
                                                        lesgammas,
                                                        lespuissancesded,
                                                        PascalRows,
                                                        IndexToR,
        ...
        m = 1
        Q, R = divmod(Mmax - 1, numworkers)
        for P in range(Q):
            useparallel = _v6_para_recurrence(m, numworkers, useparallel)
            m += numworkers
        # Ici on va invoquer une procédure parallélisée avec <numworkers, cela
        ...
        # à numworkers ?
        if R > 0:
            _ = _v6_para_recurrence(m, R, useparallel)
        ...
        if showtimes:
            ...
            map__v6_beta = _v6_map_beta_withtimes(Mmax, IndexToR, maxblock,
                                                  pool)
        else:
            map__v6_beta = _v6_map_beta_notimes(Mmax, IndexToR, maxblock,
                                                pool)

        lesbetas_maxblock0 = map__v6_beta(0)

        if k >= 1:
            lesbetas_maxblock1 = map__v6_beta(1)

        if k >= 2:
            lesbetas_maxblock2 = map__v6_beta(2)

        if (k >= 3) and (level > 2):
            lesbetas_maxblock3 = map__v6_beta(3)

        if (k >= 4) and (level > 3):
            lesbetas_maxblock4 = map__v6_beta(4)
    ...
    for j in range(0 if all else k, k+1):
        ...
        if verbose:
            ...
            if float(lastterm) == 0.:
                u, E = _v6_shorten_small_real(lastterm)
        ...
        if verbose:
            ratio = lastterm/S
            if float(ratio) == 0.:
                u, E = _v6_shorten_small_real(ratio)
...
def irwinpos(b, d, k,
    ...
    with mp.Pool(processes=numworkers) as pool:
        ...
        if showtimes:
            print("Calcul parallélisé des beta(m+1) avec "
                  f"numworkers={numworkers} ...")
            map__v6_beta = _v6_map_beta_withtimes(Mmax, IndexToR, maxblockshifted,
                                                  pool)
        else:
            map__v6_beta = _v6_map_beta_notimes(Mmax, IndexToR, maxblockshifted,
                                                pool)

        lesbetas_maxblockshifted0 = map__v6_beta(0)

        if k >= 1:
            lesbetas_maxblockshifted1 = map__v6_beta(1)

        if k >= 2:
            lesbetas_maxblockshifted2 = map__v6_beta(2)

        if (k >= 3) and (level > 2):
            lesbetas_maxblockshifted3 = map__v6_beta(3)

        if (k >= 4) and (level > 3):
            lesbetas_maxblockshifted4 = map__v6_beta(4)
...
General information is also available in the irwin_v6_docstring
variable.

The variable "numworkers" sets how many processes to use
with multiprocessing library.
You can set it either before or after loading {__filename__}.
There is no need to reload.

{numworkersinfostring}
"""
          )
    # for _ in range(10):
